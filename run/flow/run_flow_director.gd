...
func setup(_run_state: Run) -> void:
	...
	if not wave_director.wave_completed.is_connected(
		_on_wave_completed
	):
		wave_director.wave_completed.connect(
			_on_wave_completed
		)

	if not choice_director.choice_completed.is_connected(
		_on_choice_completed
	):
		choice_director.choice_completed.connect(
			_on_choice_completed
		)
...
func _on_wave_completed(wave_number: int, total_waves: int, _definition: WaveDefinition) -> void:
	_handle_wave_completed_transition.call_deferred(
		wave_number,
		total_waves
	)
...
func _handle_wave_completed_transition(wave_number: int, total_waves: int) -> void:
	...
	queue_events(
		RunEventEntryDefinition.Trigger.AFTER_WAVE,
		wave_number
	)
...
func queue_events(trigger_type: int,wave_number: int) -> void:
	...
	var events := event_schedule.get_events(
		trigger_type,
		wave_number
	)
